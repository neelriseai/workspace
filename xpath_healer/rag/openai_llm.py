...
class OpenAILLM(LLM):
    ...
    async def suggest_locators(self, prompt_payload: dict[str, Any]) -> list[dict[str, Any]]:
        system_prompt = (
            ...
            "ACTIONABILITY IS A HARD CONSTRAINT: every proposed locator must resolve to a visible and interactable "
            "element. Never propose input[type=hidden] or elements that are CSS-hidden (display:none, visibility:hidden). "
            "For custom checkbox/radio/switch controls, ALWAYS prefer the visible icon, svg, label, or wrapper "
            "element over the hidden native input[type=checkbox/radio]. The native input is typically CSS-hidden "
            "in custom UI libraries; targeting it will fail with not_visible. "
            "If rules contain require_actionable=true it means the previous candidate failed visibility check — "
            "shift all suggestions to visible wrapper/icon/label alternatives, penalising any native input selector. "
